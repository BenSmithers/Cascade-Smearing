...
class LLHMachine:
    def __init__(self):
        ...
        self._mean_error = np.sqrt(self._net_error_m**2  + self._net_error_p**2)

        self._central_llh = 1.0
        self._central_llh = self._get_log_likelihood(self.expectation)
        print("Central: {}".format(self._central_llh))

    def get_likelihood(self, flux):        
        val = self._get_log_likelihood(flux) 
        if val>self._central_llh:
            raise ValueError("Got lh greater than the central one? {}".format(val))
        return exp(val-self._central_llh)

    def _get_log_likelihood(self, flux):
        """
        ...
        for i_e in range(len(self.expectation["e_edges"])-1):
            for i_cth in range(len(self.expectation["a_edges"])-1):
                llh = llh + self._eval_llh(i_cth, i_e, flux["event_rate"][i_e][i_cth])

        return llh


    def _eval_llh(self, i_cth, i_e, bflux):
        """
